...
def trigram_document_features(document, word_features, trigram_features):
    ...
    for trigram in trigram_features:
        features['T_{}_{}_{}'.format(trigram[0], trigram[1], trigram[2])] = (trigram in document_trigrams) # Update format to accommodate for trigram terms
    return features

# 5. Combination Features - Bigram + POS
def POS_on_bigram_features(document, word_features, bigram_features):
    document_words = set(document)
    document_bigrams = nltk.bigrams(document)
    tagged_words = nltk.pos_tag(document)
    features = {}
    ...
    for bigram in bigram_features:
        features['B_{}_{}'.format(bigram[0], bigram[1])] = (bigram in document_bigrams)
        features['contains({})'.format(bigram)] = (bigram in document_bigrams)
    numNoun = 0
    numVerb = 0
    numAdj = 0
    numAdverb = 0
...
# Go through the document words in order adding the word features. If the word follows a negation words, change the feature to negated word
...
AureliaPath = r"C:\Users\aua\programsAA\IST664 - NLP\subjclueslen1-HLTEMNLP05.tff"

# this function returns a dictionary where you can look up words and get back the four items of subjectivity information described above
...
SL = readSubjectivity(AureliaPath)
...
#  print('Classifer:', BoWclassifier)
  ...
  # 4. New Feature Function: Trigrams
  print('Trigram analysis - *new feature function*')
  trigramFinder = TrigramCollocationFinder.from_words(phrase_list) # Create trigram finder on all the words in the sequence
  trigram_features = trigramFinder.nbest(trigram_measures.chi_sq, 500)
  # create feature sets for all sentences
  trigram_featuresets = [(trigram_document_features(d, BoW_features, trigram_features), c) for (d, c) in phrasedocs]

  # train trigram classifier
  # train a classifier and report accuracy
  trigram_train_set, trigram_test_set = trigram_featuresets[1000:], trigram_featuresets[:1000]
  trigramClassifier = nltk.NaiveBayesClassifier.train(trigram_train_set)
  print('Trigram classifier accuracy: ', nltk.classify.accuracy(trigramClassifier, trigram_test_set))

  # Cross-validation: Precision, recall, and F1
  cross_validation_PRF(num_folds, trigram_featuresets, labels) # cross validation accuracy to obtain precision
  print()


  # 5. Comination Feature Function: POS_on_bigram_features
  print('Combination analysis')
  # create feature sets for all sentences
  POS_on_bigram_featuresets = [(POS_on_bigram_features(d, BoW_features, bigram_features), c) for (d, c) in phrasedocs]

  # train bigram classifier
  # train a classifier and report accuracy
  POSbigram_train_set, POSbigram_test_set = POS_on_bigram_featuresets[1000:], POS_on_bigram_featuresets[:1000]
  POSbigramClassifier = nltk.NaiveBayesClassifier.train(POSbigram_train_set)
  print('Combination classifier accuracy: ', nltk.classify.accuracy(POSbigramClassifier, POSbigram_test_set))

  # Cross-validation: Precision, recall, and F1
  cross_validation_PRF(num_folds, POS_on_bigram_featuresets, labels) # cross validation accuracy to obtain precision
  print()


### Experiment: Representing negation ### 
  print('Experiment: Representing Negation')
  # Remove negation words to see if there is a change in accuracy
  # define the feature sets
  NOT_featuresets = [(NOT_features(d, BoW_features, negationwords), c) for (d, c) in phrasedocs]
  NOT_features_train_set, NOT_features_test_set = NOT_featuresets[1000:], NOT_featuresets[:1000]
  NOT_featuresclassifier = nltk.NaiveBayesClassifier.train(NOT_features_train_set)
  print('Negation classifier accuracy: ', nltk.classify.accuracy(NOT_featuresclassifier, NOT_features_test_set))
  print()

  print('Experiment: Representing Negation')
  print('Removing negation words from the data & re-running the accuracy')
  newstopwords = [word for word in nltkstopwords if word not in negationwords]
  new_all_words_list = [word for word in phrase_list if word not in newstopwords]
  removeNegationWords = nltk.FreqDist(new_all_words_list)
  removeNegationItems = removeNegationWords.most_common(1500) # find most common words
  removeNegationWordsFeatures = [word for (word, freq) in removeNegationItems] # define features from the most common words
  NOT_featuresets2 = [(NOT_features(d, removeNegationWordsFeatures, negationwords), c) for (d, c) in phrasedocs]
  NOT_features_train_set2, NOT_features_test_set2 = NOT_featuresets2[1000:], NOT_featuresets2[:1000]
  NOT_featuresclassifier2 = nltk.NaiveBayesClassifier.train(NOT_features_train_set2)
  print('Negation words removed, new classifier accuracy: ', nltk.classify.accuracy(NOT_featuresclassifier2, NOT_features_test_set2))
  print()

### Experiment: Subjectivity ### 
  print('Experiment: Subjectivity')
  SL_featuresets = [(SL_features(d, BoW_features, SL), c) for (d, c) in phrasedocs]
  SLtrain_set, SLtest_set = SL_featuresets[1000:], SL_featuresets[:1000]
  SLclassifier = nltk.NaiveBayesClassifier.train(SLtrain_set)
  print('Subjectivity classifier accuracy: ', nltk.classify.accuracy(SLclassifier, SLtest_set))
...
#  BoWgoldlist = []
#  BoWpredictedlist = []
#  for (BoW_features, label) in BoW_test_set:
#    BoWgoldlist.append(label)
#    BoWpredictedlist.append(BoWclassifier.classify(BoW_features))
#  eval_measures(BoWgoldlist, BoWpredictedlist, BoWlabels)