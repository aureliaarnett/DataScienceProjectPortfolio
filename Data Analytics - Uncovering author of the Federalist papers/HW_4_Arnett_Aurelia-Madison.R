...
# Load Corpus:
NovelsCorpusMadDispt <- Corpus(DirSource("MadisonFilesDispt"))
(getTransformations()) ## These work with library tm
(ndocs<-length(NovelsCorpusMadDispt)) ## HM & Jay docs removed

# Confirm docs have been read in
(summary(NovelsCorpusMadDispt))  ## This will list the docs in the corpus
(meta(NovelsCorpusMadDispt[[12]])) ## meta data are data hidden within a doc - like id
(meta(NovelsCorpusMadDispt[[1]],5)) ## show the ID of doc number 1
...
library("SnowballC")
Novels_dtm_MadDispt <- DocumentTermMatrix(NovelsCorpusMadDispt,
                                          control = list(
...
(inspect(Novels_dtm_MadDispt))  ## This takes a look at a subset - a peak
DTM_mat <- as.matrix(Novels_dtm_MadDispt)
(DTM_mat[1:13,1:10])


(WordFreq <- colSums(as.matrix(Novels_dtm_MadDispt)))
...
(WordFreq[tail(ord)])
## Row Sums
(Row_Sum_Per_doc <- rowSums((as.matrix(Novels_dtm_MadDispt))))


## Copy of a matrix format of the data
Novels_M <- as.matrix(Novels_dtm_MadDispt)
(Novels_M[1:13,1:5])
...
Novels_DF <- as.data.frame(as.matrix(Novels_dtm_MadDispt))
length(Novels_DF)
(head(Novels_DF))
str(Novels_DF)
(Novels_DF$abolit)
